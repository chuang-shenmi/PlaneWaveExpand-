function [tau_power, tau_pressure, W2_G, Kz, G, feature] = PWE4PlateResonatorPlateDimless2DOF_Matrix(plate,resonator,lattice,medium,incident,M,freq,meta)
...
omega = 2*pi*freq;
k0 = omega/medium.c0;
% 正格矢R（m_=1,n_=1）计算时不用正格矢，仅用于生成倒格矢，只需给出系数为一的
...
for i = 1:nfreq
    ...
    % 对于板的控制方程中的矩阵（PWE展开后）
    Kirchoff.K_1p = zeros(N_PW,N_PW);  % 上板的刚度矩阵
    Kirchoff.K_2p = zeros(N_PW,N_PW);  % 下板的刚度矩阵
    Kirchoff.C_1f = zeros(N_PW,N_PW);  % 对于上板的流体负载矩阵
    Kirchoff.C_2f = zeros(N_PW,N_PW);  % 对于下板的流体负载矩阵
    Kirchoff.M_1p = zeros(N_PW,N_PW);  % 上板的质量矩阵
    Kirchoff.M_2p = zeros(N_PW,N_PW);  % 下板的质量矩阵
    Kirchoff.D_r11 = zeros(N_PW,N_PW);  % 谐振器动态刚度系数矩阵D_r,11
    Kirchoff.D_r12 = zeros(N_PW,N_PW);  % 谐振器动态刚度系数矩阵D_r,12
    Kirchoff.D_r21 = zeros(N_PW,N_PW);  % 谐振器动态刚度系数矩阵D_r,21
    Kirchoff.D_r22 = zeros(N_PW,N_PW);  % 谐振器动态刚度系数矩阵D_r,22
    Kirchoff.F1 = zeros(N_PW,1);  % 激励向量
    ...
    if nargin >= 8 && isstruct(meta) && isfield(meta,'l_PML') && meta.l_PML > 0
        ...
        % PML core
        [Kz(:,:,i), Kirchoff] = PMLbaseFFT(omega(i), G, medium, meta, incident, Kirchoff);
    end
