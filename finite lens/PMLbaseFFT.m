function [Kz, Kirchoff] = PMLbaseFFT(omega, G, medium, plate, meta, incident, Kirchoff)
...
switch meta.type
    ...
    case '2D'
        ...
        zeta_y(mask_y) = meta.zeta_max * ((abs(ETA(mask_y)) - meta.Ly_lens/2) / meta.l_PML).^3;
    otherwise
        error('Unknown PML type: %s', meta.type);
end
...
% Inverse stretch functions
Sx_inv = 1 ./ Sx;
Sy_inv = 1 ./ Sy;

% 2D-FFT to extract expansion coefficients
Sx_inv_fft = fftshift(ifft2(ifftshift(Sx_inv)));
Sy_inv_fft = fftshift(ifft2(ifftshift(Sy_inv)));

% Build Toeplitz matrices using purely integer dimensionless indices
Sx_inv_Toeplitz = zeros(N_PW, N_PW);
Sy_inv_Toeplitz = zeros(N_PW, N_PW);
...
for m_idx = 1:N_PW
    for n_idx = 1:N_PW
        ...
        if idx_x > 0 && idx_x <= Nx_grid && idx_y > 0 && idx_y <= Ny_grid
            % Assign Toeplitz elements
            Sx_inv_Toeplitz(m_idx, n_idx) = Sx_inv_fft(idx_y, idx_x);
            Sy_inv_Toeplitz(m_idx, n_idx) = Sy_inv_fft(idx_y, idx_x);
        end
    end
end
...
Kirchoff.F1 = GaussianBeamExcitation(meta, N_PW, G, incident);


Kx = diag(kx+G(:,1));
Ky = diag(ky+G(:,2));
Dx = Sx_inv_Toeplitz * (-1i * Kx);
Dy = Sy_inv_Toeplitz * (-1i * Ky);
% Convolve the Toeplitz matrices with your original diagonal operators
D_Toeplitz = ((Dx * Dx) + (Dy * Dy))^2;
Kirchoff.K_1p = D_Toeplitz;
Kirchoff.K_2p = plate.D(2)/plate.D(1) * D_Toeplitz;
% Couple PML matrix (K_zG)
